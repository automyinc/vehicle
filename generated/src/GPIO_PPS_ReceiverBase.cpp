
// AUTO GENERATED by vnxcppcodegen

#include <vehicle/package.hxx>
#include <vehicle/GPIO_PPS_ReceiverBase.hxx>
#include <vnx/Input.h>
#include <vnx/Output.h>
#include <vnx/Visitor.h>
#include <vnx/Object.h>
#include <vnx/Struct.h>
#include <vnx/Config.h>


namespace vehicle {


const vnx::Hash64 GPIO_PPS_ReceiverBase::VNX_TYPE_HASH(0x3abd359cd4c29f75ull);
const vnx::Hash64 GPIO_PPS_ReceiverBase::VNX_CODE_HASH(0x6b037d34fbcefb50ull);

GPIO_PPS_ReceiverBase::GPIO_PPS_ReceiverBase(const std::string& _vnx_name)
	:	Module::Module(_vnx_name)
{
	vnx::read_config(vnx_name + ".device", device);
	vnx::read_config(vnx_name + ".output", output);
}

vnx::Hash64 GPIO_PPS_ReceiverBase::get_type_hash() const {
	return VNX_TYPE_HASH;
}

const char* GPIO_PPS_ReceiverBase::get_type_name() const {
	return "vehicle.GPIO_PPS_Receiver";
}

void GPIO_PPS_ReceiverBase::accept(vnx::Visitor& _visitor) const {
	const vnx::TypeCode* _type_code = get_type_code();
	_visitor.type_begin(*_type_code);
	_visitor.type_field(_type_code->fields[0], 0); vnx::accept(_visitor, output);
	_visitor.type_field(_type_code->fields[1], 1); vnx::accept(_visitor, device);
	_visitor.type_end(*_type_code);
}

void GPIO_PPS_ReceiverBase::write(std::ostream& _out) const {
	_out << "{";
	_out << "\"output\": "; vnx::write(_out, output);
	_out << ", \"device\": "; vnx::write(_out, device);
	_out << "}";
}

void GPIO_PPS_ReceiverBase::read(std::istream& _in) {
	std::map<std::string, std::string> _object;
	vnx::read_object(_in, _object);
	for(const auto& _entry : _object) {
		if(_entry.first == "device") {
			vnx::from_string(_entry.second, device);
		} else if(_entry.first == "output") {
			vnx::from_string(_entry.second, output);
		}
	}
}

vnx::Object GPIO_PPS_ReceiverBase::to_object() const {
	vnx::Object _object;
	_object["output"] = output;
	_object["device"] = device;
	return _object;
}

void GPIO_PPS_ReceiverBase::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		if(_entry.first == "device") {
			_entry.second.to(device);
		} else if(_entry.first == "output") {
			_entry.second.to(output);
		}
	}
}

/// \private
std::ostream& operator<<(std::ostream& _out, const GPIO_PPS_ReceiverBase& _value) {
	_value.write(_out);
	return _out;
}

/// \private
std::istream& operator>>(std::istream& _in, GPIO_PPS_ReceiverBase& _value) {
	_value.read(_in);
	return _in;
}

const vnx::TypeCode* GPIO_PPS_ReceiverBase::get_type_code() {
	const vnx::TypeCode* type_code = vnx::get_type_code(vnx::Hash64(0x3abd359cd4c29f75ull));
	if(!type_code) {
		type_code = vnx::register_type_code(create_type_code());
	}
	return type_code;
}

std::shared_ptr<vnx::TypeCode> GPIO_PPS_ReceiverBase::create_type_code() {
	std::shared_ptr<vnx::TypeCode> type_code = std::make_shared<vnx::TypeCode>(true);
	type_code->name = "vehicle.GPIO_PPS_Receiver";
	type_code->type_hash = vnx::Hash64(0x3abd359cd4c29f75ull);
	type_code->code_hash = vnx::Hash64(0x6b037d34fbcefb50ull);
	type_code->methods.resize(0);
	type_code->fields.resize(2);
	{
		vnx::TypeField& field = type_code->fields[0];
		field.is_extended = true;
		field.name = "output";
		field.value = vnx::to_string("vehicle.pps_signal");
		field.code = {12, 5};
	}
	{
		vnx::TypeField& field = type_code->fields[1];
		field.is_extended = true;
		field.name = "device";
		field.code = {12, 5};
	}
	type_code->build();
	return type_code;
}

void GPIO_PPS_ReceiverBase::handle_switch(std::shared_ptr<const ::vnx::Sample> _sample) {
	const uint64_t _type_hash = _sample->value->get_type_hash();
}

bool GPIO_PPS_ReceiverBase::call_switch(vnx::TypeInput& _in, vnx::TypeOutput& _out, const vnx::TypeCode* _call_type, const vnx::TypeCode* _return_type) {
	return false;
}


} // namespace vehicle


namespace vnx {

void read(TypeInput& in, ::vehicle::GPIO_PPS_ReceiverBase& value, const TypeCode* type_code, const uint16_t* code) {
	if(!type_code) {
		throw std::logic_error("read(): type_code == 0");
	}
	if(code) {
		switch(code[0]) {
			case CODE_STRUCT: type_code = type_code->depends[code[1]]; break;
			case CODE_ALT_STRUCT: type_code = type_code->depends[vnx::flip_bytes(code[1])]; break;
			default: vnx::skip(in, type_code, code); return;
		}
	}
	const char* const _buf = in.read(type_code->total_field_size);
	for(const vnx::TypeField* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			case 0: vnx::read(in, value.output, type_code, _field->code.data()); break;
			case 1: vnx::read(in, value.device, type_code, _field->code.data()); break;
			default: vnx::skip(in, type_code, _field->code.data());
		}
	}
}

void write(TypeOutput& out, const ::vehicle::GPIO_PPS_ReceiverBase& value, const TypeCode* type_code, const uint16_t* code) {
	if(!type_code || (code && code[0] == CODE_ANY)) {
		type_code = vnx::write_type_code<::vehicle::GPIO_PPS_ReceiverBase>(out);
		vnx::write_class_header<::vehicle::GPIO_PPS_ReceiverBase>(out);
	}
	if(code && code[0] == CODE_STRUCT) {
		type_code = type_code->depends[code[1]];
	}
	vnx::write(out, value.output, type_code, type_code->fields[0].code.data());
	vnx::write(out, value.device, type_code, type_code->fields[1].code.data());
}

void read(std::istream& in, ::vehicle::GPIO_PPS_ReceiverBase& value) {
	value.read(in);
}

void write(std::ostream& out, const ::vehicle::GPIO_PPS_ReceiverBase& value) {
	value.write(out);
}

void accept(Visitor& visitor, const ::vehicle::GPIO_PPS_ReceiverBase& value) {
	value.accept(visitor);
}

} // vnx
